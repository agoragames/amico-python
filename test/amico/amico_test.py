...
class AmicoTest(unittest.TestCase):
    ...
    def test_it_should_remove_the_pending_relationship_and_add_to_following_and_followers_if_accept_is_called(
            self):
        amico = Amico(
            options={
                'pending_follow': True},
    ...
    def test_it_should_allow_you_to_block_someone_who_is_not_following_you(
            self):
        amico = Amico(redis_connection=self.redis_connection)
        amico.block(1, 11)
    ...
    def test_it_should_return_the_correct_count_for_various_types_of_relationships(
            self):
        amico = Amico(redis_connection=self.redis_connection)
    ...
    # helper methods
    def __add_reciprocal_followers(
            self,
            amico,
            count=27,
            block_relationship=False):
