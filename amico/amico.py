...
class Amico(object):
    ...
    def follow(self, from_id, to_id, scope=None):
        """
        Establish a follow relationship between two IDs. After adding the
        follow
        relationship, it checks to see if the relationship is reciprocated and
        establishes that
        relationship if so.

        @param from_id [String] The ID of the individual establishing the
        follow relationship.
        ...
        if self.is_blocked(to_id, from_id, scope):
            return
        if self.options['pending_follow'] and self.is_pending(from_id,
                                                              to_id, scope):
            return
    ...
    def unfollow(self, from_id, to_id, scope=None):
        ...
        @param from_id [String] The ID of the individual removing the follow
        relationship.
        ...
        transaction = self.redis_connection.pipeline()
        transaction.zrem('%s:%s:%s:%s' % (self.options['namespace'],
    ...
    def block(self, from_id, to_id, scope=None):
        """
        Block a relationship between two IDs. This method also has the side
        effect
        of removing any follower or following relationship between the two IDs.

        @param from_id [String] The ID of the individual blocking the
        relationship.
    ...
    def unblock(self, from_id, to_id, scope=None):
        ...
        @param from_id [String] The ID of the individual unblocking the
        relationship.
        ...
        transaction = self.redis_connection.pipeline()
        transaction.zrem('%s:%s:%s:%s' % (self.options['namespace'],
                                          self.options['blocked_key'], scope,
                                          from_id), to_id)
        transaction.zrem('%s:%s:%s:%s' % (self.options['namespace'],
                                          self.options['blocked_by_key'],
                                          scope, to_id), from_id)
    ...
    def accept(self, from_id, to_id, scope=None):
        ...
        @param from_id [String] The ID of the individual accepting the
        relationship.
    ...
    def deny(self, from_id, to_id, scope=None):
        ...
        @param from_id [String] The ID of the individual denying the
        relationship.
    ...
    def clear(self, id, scope=None):
        """
        Clears all relationships (in either direction) stored for an
        individual.
    ...
    def is_blocked(self, id, blocked_id, scope=None):
        ...
        @param blocked_id [String] ID of the individual to see if they are
        blocked by id.
        @param scope [String] Scope for the call.
        """
        if not scope:
    ...
    def is_blocked_by(self, id, blocked_by_id, scope=None):
        ...
        @param blocked_by_id [String] ID of the individual to see if they have
        blocked id.
        @param scope [String] Scope for the call.
        """
        if not scope:
    ...
    def is_follower(self, id, follower_id, scope=None):
        ...
        @param follower_id [String] ID of the individual to see if they are
        following id.
        @param scope [String] Scope for the call.
        """
        if not scope:
    ...
    def is_following(self, id, following_id, scope=None):
        ...
        @param following_id [String] ID of the individual to see if they are
        being followed by id.
        @param scope [String] Scope for the call.
        """
        if not scope:
            scope = self.options['default_scope_key']
    ...
    def is_reciprocated(self, from_id, to_id, scope=None):
        """
        Check to see if one individual has reciprocated in following another
        individual.

        @param from_id [String] ID of the individual checking the reciprocated
        relationship.
        @param to_id [String] ID of the individual to see if they are following
        from_id.
        @param scope [String] Scope for the call.
        """
        if not scope:
        ...
        return self.is_following(from_id, to_id, scope) and \
            self.is_following(to_id, from_id, scope)

    def is_pending(self, from_id, to_id, scope=None):
        """
        Check to see if one individual has a pending relationship in following
        another individual.

        @param from_id [String] ID of the individual checking the pending
        relationships.
        @param to_id [String] ID of the individual to see if they are pending a
        follow from from_id.
        @param scope [String] Scope for the call.
        """
        if not scope:
    ...
    def is_pending_with(self, from_id, to_id, scope=None):
        ...
        @param from_id [String] ID of the individual checking the pending
        relationships.
        @param to_id [String] ID of the individual to see if they are pending
        an approval from from_id.
        @param scope [String] Scope for the call.
        """
        if not scope:
            scope = self.options['default_scope_key']
    ...
    def following_count(self, id, scope=None):
        ...
        @param id [String] ID of the individual to retrieve following count
        for.
        @param scope [String] Scope for the call.
        """
        if not scope:
    ...
    def followers_count(self, id, scope=None):
        ...
        @param id [String] ID of the individual to retrieve followers count
        for.
        @param scope [String] Scope for the call.
        """
        if not scope:
    ...
    def blocked_by_count(self, id, scope=None):
        ...
        @param id [String] ID of the individual to retrieve blocked_by count
        for.
        @param scope [String] Scope for the call.
        """
        if not scope:
    ...
    def reciprocated_count(self, id, scope=None):
        """
        Count the number of individuals that have reciprocated a following
        relationship.

        @param id [String] ID of the individual to retrieve reciprocated
        following count for.
        @param scope [String] Scope for the call.
        """
        if not scope:
    ...
    def pending_with_count(self, id, scope=None):
        """
        Count the number of relationships an individual has pending with
        another.
    ...
    def following(self, id, page_options=None, scope=None):
        ...
        @param page_options [Hash] Options to be passed for retrieving a page
        of followed individuals.
        @param scope [String] Scope for the call.
        """
        if not scope:
            scope = self.options['default_scope_key']
    ...
    def reciprocated(self, id, page_options=None, scope=None):
        """
        Retrieve a page of individuals that have reciprocated a follow for a
        given ID.

        @param id [String] ID of the individual.
        @param page_options [Hash] Options to be passed for retrieving a page
        of individuals that have reciprocated a follow.
        @param scope [String] Scope for the call.
        """
        if not scope:
            scope = self.options['default_scope_key']
    ...
    def pending_with(self, id, page_options=None, scope=None):
        """
        Retrieve a page of individuals that are waiting to approve the given
        ID.
    ...
    def followers_page_count(self, id, page_size=None, scope=None):
        ...
        @param id [String] ID of the individual.
        @param page_size [int] Page size (default:
        Amico.DEFAULTS['page_size']).
        @param scope [String] Scope for the call.
        """
        if not scope:
            scope = self.options['default_scope_key']
    ...
    def blocked_by_page_count(self, id, page_size=None, scope=None):
        """
        Count the number of pages of blocked_by relationships for an
        individual.
    ...
    def reciprocated_page_count(self, id, page_size=None, scope=None):
        """
        Count the number of pages of reciprocated relationships for an
        individual.
    ...
    def pending_with_page_count(self, id, page_size=None, scope=None):
        """
        Count the number of pages of individuals waiting to approve another
        individual.
    ...
    def all(self, id, type, scope=None):
        """
        Retrieve all of the individuals for a given id, type (e.g. following)
        and scope

        @param id [String] ID of the individual.
        @param type [String] One of 'following', 'followers', 'reciprocated',
        'blocked', 'blocked_by', 'pending', 'pending_with'.
        @param scope [String] Scope for the call.
        """
        if not scope:
            scope = self.options['default_scope_key']
        ...
        if count > 0:
            return getattr(self, '%s' % type)(id, {'page_size': count,
                                                   'page': 1}, scope)
        else:
            return []
    ...
    def count(self, id, type, scope=None):
        """
        Retrieve a count of all of a given type of relationship for the
        specified id.
    ...
    def page_count(self, id, type, page_size=None, scope=None):
        """
        Retrieve a page count of a given type of relationship for the specified
        id.
    ...
    # etc...
    VALID_RELATIONSHIPS = ['following', 'followers', 'reciprocated', 'blocked',
                           'blocked_by', 'pending', 'pending_with']
    ...
    def __validate_relationship_type(self, type):
        ...
        @param type [String] One of 'following', 'followers', 'reciprocated',
        'blocked', 'blocked_by', 'pending', 'pending_with'.
        @raise [StandardError] if the type is not included in
        VALID_RELATIONSHIPS
        """
        if type not in self.VALID_RELATIONSHIPS:
            raise Exception('Invalid relationship type given %s' % type)

    def __clear_bidirectional_sets_for_id(self, id, source_set_key,
                                          related_set_key, scope=None):
        """
        Removes references to an individual in sets that are named with other
        individual's keys.
        Assumes two set keys that are used together such as
        followers/following, blocked/blocked_by, etc...

        @param id [String] The ID of the individual to clear info for.
        @param source_set_key [String] The key identifying the souce set to
        iterate over.
        @param related_set_key [String] The key identifying the sets that the
        idividual needs to be removed from.
        @param scope [String] Scope for the call.
        """
        if not scope:
            scope = self.options['default_scope_key']
    ...
    def __add_following_followers_reciprocated(self, from_id, to_id,
                                               scope=None):
        """
        Add the following, followers and check for a reciprocated relationship.
        To be used from the
    ...
    def __total_pages(self, key, page_size):
        ...
        """
        return int(
            math.ceil(self.redis_connection.zcard(key) / float(page_size)))
